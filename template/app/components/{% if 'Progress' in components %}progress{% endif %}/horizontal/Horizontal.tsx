...
import {
  type UseScaleUtilsReturnType,
  useScaleUtils,
  useCurrentThemeMode,
  ThemeModeEnum
} from 'rn-custom-style-sheet';
import { useStatusBarHeight } from '@hooks';
import { Colors, windowWidth } from '@themes';
  ...
  return (
    ...
    >
      <Progress.Bar
        ...
        height={scale(5)}
        color={colorOpacity(Colors[themeMode]?.secondary, 0.5)}
        unfilledColor={colorOpacity(Colors[themeMode]?.invertedBlack, 0.5)}
        borderColor={Colors[themeMode]?.secondary}
        {...props}
      />
    </View>
  );
