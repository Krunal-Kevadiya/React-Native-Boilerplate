 ...
 * @returns {UserResponse | null | undefined} - the user from the local state, or undefined if the user is not logged in
 */
const getLocalUser = (state: RootStateType): UserResponse | null | undefined => state.auth.user;
/**
 * Gets the error from the state.
 * @param {RootStateType} state - the state object
 * @returns {string | null | undefined} - the error object, if there is one.
 */
const getLocalError = (state: RootStateType): string | null | undefined => state.auth.error;
 ...
 * @property {(state: RootStateType) => string} getError - The selector for the error state.
...
type AuthSelectorsType = {
  ...
  getError: (state: RootStateType) => string;
...
const AuthSelectors: AuthSelectorsType = {
  ...
  getUser: createDraftSafeSelector(getLocalUser, (user: UserResponse | null | undefined) =>
