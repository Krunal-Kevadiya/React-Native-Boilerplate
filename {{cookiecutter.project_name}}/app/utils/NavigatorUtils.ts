...
import { DeepLinkEnum, deepLinkPrefixes, AppRouteEnum } from '@constants';
import {
  checkAndGetParams,
  isDeepLinkType,
...
function handleUrlLink(nonBranchUrl: string): string | undefined {
  ...
  if (details !== undefined) {
    const isToastMessage = isDeepLinkType(
      details.routeName,
      details.params,
      DeepLinkEnum.ToastMessage,
      true
    );
    if (
      isDeepLinkType(details.routeName, details.params, DeepLinkEnum.ToastMessage) &&
...
export function getLinkConfiguration(): LinkingOptions<ReactNavigation.RootParamList> | undefined {
  const linking = {
    ...
    async getInitialURL() {
      ...
      return url && isPresentValue(url) ? handleUrlLink(url) : url;
    },
    ...
    subscribe: (listener: (url: string) => void) => {
      ...
      const onReceiveURL = ({ url }: { url: string }) => {
        listener((url && isPresentValue(url) ? handleUrlLink(url) : url) ?? '');
      };
    ...
    config: {
      ...
      screens: {
        ...
        [AppRouteEnum.AUTH]: {
          ...
          screens: {
            [AppRouteEnum.SIGN_IN]: 'signIn/:link?/:tenantId?/:email?',
            [AppRouteEnum.CREATE_NEW_PASSWORD]: 'createNewPassword/:code?/:tenantId?/:email?'
          }
        },
        [AppRouteEnum.HOME]: {
          path: 'home'
          /*initialRouteName: AppRouteEnum.Dashboard,
          screens: {
            [ROUTES.Dashboard]: {
              path: 'dashboard',
